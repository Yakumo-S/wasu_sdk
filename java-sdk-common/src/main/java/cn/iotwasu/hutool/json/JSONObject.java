...
import cn.iotwasu.hutool.json.serialize.GlobalSerializeMapping;
import cn.iotwasu.hutool.json.serialize.JSONObjectSerializer;
import cn.iotwasu.hutool.json.serialize.JSONSerializer;
import cn.iotwasu.hutool.core.bean.BeanDesc.PropDesc;
import cn.iotwasu.hutool.core.bean.BeanPath;
...
import cn.iotwasu.hutool.core.map.CaseInsensitiveLinkedMap;
import cn.iotwasu.hutool.core.map.CaseInsensitiveMap;
...
public class JSONObject extends JSONGetter<String> implements JSON, Map<String, Object> {
  ...
  public JSONObject(int capacity, JSONConfig config) {
    if (config.isIgnoreCase()) {
      this.rawHashMap =
          config.isOrder()
              ? new CaseInsensitiveLinkedMap<String, Object>(capacity)
              : new CaseInsensitiveMap<String, Object>(capacity);
    } else {
      this.rawHashMap =
          config.isOrder()
              ? new LinkedHashMap<String, Object>(capacity)
              : new HashMap<String, Object>(capacity);
    }
    this.config = config;
  }
   ...
   *
   * <ol>
   *   <li>value为Map，将键值对加入JSON对象
   *   <li>value为JSON字符串（CharSequence），使用JSONTokener解析
   *   <li>value为JSONTokener，直接解析
   *   <li>value为普通JavaBean，如果为普通的JavaBean，调用其getters方法（getXXX或者isXXX）获得值，加入到JSON对象。例如：如果JavaBean对象中有个方法getName()，值为"张三"，获得的键值对为：name:
   *       "张三"
   * </ol>
   *
        ...
        JSONConfig.create()
            ...
            .setIgnoreCase(
                (source instanceof CaseInsensitiveMap)
                    || (source instanceof CaseInsensitiveLinkedMap)) //
  ...
  private Writer doWrite(Writer writer, int indentFactor, int indent) throws IOException {
    ...
    for (Entry<String, Object> entry : this.entrySet()) {
      if (ObjectUtil.isNull(entry.getKey())
          || (ObjectUtil.isNull(entry.getValue()) && isIgnoreNullValue)) {
        continue;
      }
