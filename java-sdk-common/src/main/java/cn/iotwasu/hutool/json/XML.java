...
public class XML {
        ...
        if (token instanceof String) {
          ...
          if (token == EQ) {
            ...
            jsonobject.accumulate(
                string, keepStrings ? token : InternalJSONUtil.stringToValue((String) token));
            token = null;
          } else {
          ...
          for (; ; ) {
            ...
            if (token == null) {
              ...
              if (string.length() > 0) {
                jsonobject.accumulate(
                    "content", keepStrings ? token : InternalJSONUtil.stringToValue(string));
              }
   ...
   * in a "content" member. Comments, prologs, DTDs, and <code>&lt;[ [ ]]&gt;</code> are ignored.
   *
