package cn.iotwasu.hutool.core.swing.clipboard;
...
public enum ClipboardMonitor implements ClipboardOwner, Runnable, Closeable {
  ...
  public void lostOwnership(Clipboard clipboard, Transferable contents) {
    ...
    for (ClipboardListener listener : listenerSet) {
      try {
        transferable =
            listener.onChange(clipboard, ObjectUtil.defaultIfNull(transferable, newContents));
      } catch (Throwable e) {
        // 忽略事件处理异常，保证所有监听正常执行
      }
    }
    ...
    if (isRunning) {
      ...
      clipboard.setContents(
          ObjectUtil.defaultIfNull(transferable, ObjectUtil.defaultIfNull(newContents, contents)),
          this);
    }
  }
