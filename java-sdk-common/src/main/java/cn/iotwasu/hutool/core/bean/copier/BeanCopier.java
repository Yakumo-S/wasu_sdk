package cn.iotwasu.hutool.core.bean.copier;

import cn.iotwasu.hutool.core.bean.copier.provider.BeanValueProvider;
import cn.iotwasu.hutool.core.bean.copier.provider.MapValueProvider;
...
import cn.iotwasu.hutool.core.lang.ParameterizedTypeImpl;
import cn.iotwasu.hutool.core.lang.copier.Copier;
...
public class BeanCopier<T> implements Copier<T>, Serializable {
  ...
  public static <T> BeanCopier<T> create(
      Object source, T dest, Type destType, CopyOptions copyOptions) {
  ...
  private void beanToBean(Object providerBean, Object destBean) {
    valueProviderToBean(
        new BeanValueProvider(
            providerBean, this.copyOptions.ignoreCase, this.copyOptions.ignoreError),
        destBean);
  }
  ...
  private void beanToMap(Object bean, Map targetMap) {
    ...
    final HashSet<String> ignoreSet =
        (null != copyOptions.ignoreProperties)
    ...
    for (PropDesc prop : props) {
      ...
      if (firstParamType instanceof ParameterizedType) {
        ...
        if (TypeUtil.hasTypeVeriable(actualTypeArguments)) {
          // 泛型对象中含有未被转换的泛型变量
          actualTypeArguments =
              TypeUtil.getActualTypes(
                  this.destType, setterMethod.getDeclaringClass(), tmp.getActualTypeArguments());
          if (ArrayUtil.isNotEmpty(actualTypeArguments)) {
            // 替换泛型变量为实际类型
            firstParamType =
                new ParameterizedTypeImpl(
                    actualTypeArguments, tmp.getOwnerType(), tmp.getRawType());
          }
        }
