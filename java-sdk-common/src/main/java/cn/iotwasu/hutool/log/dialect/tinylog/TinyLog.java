...
public class TinyLog extends AbstractLog {
  ...
  public void log(
      String fqcn,
      cn.iotwasu.hutool.log.level.Level level,
      Throwable t,
      String format,
      Object... arguments) {
  ...
  @Override
  public boolean isEnabled(cn.iotwasu.hutool.log.level.Level level) {
  ...
  private void logIfEnabled(
      String fqcn, Level level, Throwable t, String format, Object... arguments) {
    // fqcn 无效
    if (null == t) {
   ...
   * @since 4.0.3
   */
  private Level toTinyLevel(cn.iotwasu.hutool.log.level.Level level) {
    Level tinyLevel;
    switch (level) {
      case TRACE:
