package cn.iotwasu.hutool.http.webservice;
...
import cn.iotwasu.hutool.core.util.XmlUtil;
import cn.iotwasu.hutool.http.HttpGlobalConfig;
 ...
 * SOAP客户端
 *
 * <p>此对象用于构建一个SOAP消息，并通过HTTP接口发出消息内容。 SOAP消息本质上是一个XML文本，可以通过调用{@link #getMsgStr(boolean)} 方法获取消息体
 *
 * <p>使用方法：
 *
 * <pre>
...
public class SoapClient {
   ...
   * 重置SOAP客户端，用于客户端复用
   *
   * <p>重置后需调用serMethod方法重新指定请求方法，并调用setParam方法重新定义参数
   *
   * @return this
   * @since 4.6.7
   */
   ...
   * @return this
   */
  public SoapClient setHeader(
      QName name, String actorURI, String roleUri, Boolean mustUnderstand, Boolean relay) {
    SOAPHeader header;
  ...
  public SoapClient setMethod(Name name, Map<String, Object> params, boolean useMethodPrefix) {
    return setMethod(
        new QName(name.getURI(), name.getLocalName(), name.getPrefix()), params, useMethodPrefix);
  }
  ...
  public SoapClient setMethod(String methodName) {
    return setMethod(
        methodName, ObjectUtil.defaultIfNull(this.namespaceURI, XMLConstants.NULL_NS_URI));
  }
