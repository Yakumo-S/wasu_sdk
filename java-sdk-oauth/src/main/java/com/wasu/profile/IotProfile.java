...
public class IotProfile {
  ...
  private void initSystemVersion() {
    ...
    try {
      ...
      logger.error(
          "init iot subsystem version error [{}],error [{}]", URL_SCHEME + BRM_BASE_VERSION_URL, e);
      e.printStackTrace();
    }
    try {
      ...
      if (StrUtil.isNotBlank(baseVersion)) {
        ...
        IotHttpRequest subSystemRequest =
            new IotHttpRequest(String.format(URL_SCHEME + BRM_VERSION_URL, baseVersion));
